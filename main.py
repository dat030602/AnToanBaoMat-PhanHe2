...
class NhanVien_controller:
    ...
    def update_info(self, manv, ngaysinh, diachi, sodt):
        sql = "UPDATE nvquantri.CS_NHANVIEN SET NgaySinh = '{0}', DiaChi = '{1}', SoDT = '{2}' WHERE MANV = '{3}'".format(ngaysinh, diachi, sodt, manv)
...
class DeAn_controller:
    ...
    def update_info(self, mada, tenda, ngaybd, phong):
        sql = "UPDATE nvquantri.DEAN SET TENDA = '{0}', NGAYBD = '{1}', PHONG = '{2}' WHERE MADA = '{3}'".format(tenda, ngaybd, phong, mada)
        result, er = execute_query(login_info[0], login_info[1], sql)
        return result ,er
    ...
    def add_info(self, mada, tenda, ngaybd, phong):
        sql = "INSERT INTO nvquantri.DEAN (MADA, TENDA, NGAYBD, PHONG) VALUES ('{0}', '{1}', '{2}', '{3}')".format(mada, tenda, ngaybd, phong)
        result, er = execute_query(login_info[0], login_info[1], sql)
        return result ,er
...
class Taichinh_controller:
    ...
    def update_info(self, manv, ngaysinh, diachi, sodt):
        sql = "UPDATE nvquantri.NHANVIEN SET NgaySinh = '{0}', DiaChi = '{1}', SoDT = '{2}' WHERE MANV = '{3}'".format(ngaysinh, diachi, sodt, manv)
        result, er = execute_query(login_info[0], login_info[1], sql)
        return result ,er
    
    def update_nhanvien(self, manv, luong, phucap):
        sql = "UPDATE nvquantri.NHANVIEN SET LUONG = '{0}', PHUCAP = '{1}' WHERE MANV = '{2}'".format(luong, phucap, manv)
        result, er = execute_query(login_info[0], login_info[1], sql)
        return result ,er
...
class Nhanvien_ThongTinCaNhan:
    ...
    def click_update(self):
        ...
        window_nhanvien_update.Load_Data(self.manv, self.ngaysinh, self.diachi, self.sodt)
...
class update_thongtincanhan:
    ...
    def clicked_update(self):
        ...
        result,er = controller.update_info(manv_new, ngaysinh_new, diachi_new, sodt_new)
        if result == False:
            if er == 0:
...
class Taichinh_view:
    def Load_Data(self):
        ...
        # thiết lập hover cursor
        self.button_assign.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        # self.button_assign.clicked.connect(self.clicked_logout)
...
class TaiChinh_ThongTinNhanVien:
    ...
    def clicked_update(self):
        ...
        result , er = controller.update_nhanvien(manv_new, luong_new, phucap_new)
        self.update_info()
...
class TaiChinh_ThongTinCaNhan:
    ...
    def click_update(self):
        ...
        window_taichinh_update.Load_Data(self.manv, self.ngaysinh, self.diachi, self.sodt)
...
class DanhSachDeAn_view:
    ...
    def click_update(self):
        ...
        if self.MADA == '':
            ...
            return
        window_dean_update.Load_Data(self.MADA, self.TENDA, self.NGAYBD, self.PHONG)
...
class add_dean:
    ...
    def clicked_add(self):
        ...
        result,er = controller.add_info(mada_new, tenda_new, ngaybd_new, phong_new)
...
class update_dean:
    ...
    def clicked_update(self):
        ...
        result,er = controller.update_info(mada_new, tenda_new, ngaybd_new, phong)
        if result == False:
            if er == 0:
                MessageBoxInfo("Thông Báo", "Cập nhật thành công")
            if er == 12899:
